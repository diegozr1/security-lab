...
FILE *DestFile;
unsigned char Buffer[3][3];  // This is a 8 bits long variable without sign.  We will
                       // used it as a Byte.
...
unsigned char headers;
  ...
  if ( SourceFile != NULL ) {
    ...
    if ( DestFile != NULL ) {
      ...
      int i,j;
      for(i=0; i <54; i++){
        fread(&Buffer[0][0],1,1, SourceFile);
        fwrite(&Buffer[0][0],1,1, DestFile);
      }
      while(fread(Buffer, 9,1, SourceFile)!=0){
        for(i=0; i < 3; i++){
          for(j=0; j < 3; j++){
            fwrite(&Buffer[j][i],1,1,DestFile);
          }
        }
      }
